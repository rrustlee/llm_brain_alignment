...
log_name = f'reverse_window_{args.window}-layer_{args.layer}-{args.act_name}.txt'
...
rstim, tr_stats, word_stats = get_stim_torch(args, stories, llama)
rresp = get_resp_torch(args.subject, stories, stack = True)
rstim = rstim.cuda()
rresp = rresp.cuda()
...
# stim_dict = get_resp(args.subject, stories, stack = False, delay=True, vox = vox)
# resp_dict = {story : get_stim(args, [story], llama, delay=False)[0] for story in stories}
stim_dict = get_resp_torch(args.subject, stories, stack = False, delay=True, vox = vox)
resp_dict = {story : get_stim_torch(args, [story], llama, delay=False)[0] for story in stories}
